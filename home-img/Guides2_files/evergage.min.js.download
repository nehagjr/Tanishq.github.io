...
Evergage.init({            
                                ...
                                Evergage.sendEvent(
                                    {
                                        ...
                                        user:
                                        {
                                            attributes: { emailAddress: email_id }
                                        }
                                    }
                                );
                                ...
                                Evergage.sendEvent(
                                    {
                                        ...
                                        user:
                                        {
                                            attributes: { phoneNumber: mobile_number }
                                        }
                                    }
                                );
 ...
                     actionEvent.user.attributes.phoneNumber = mobile_number;
            ...
            pageTypes: [
                {
                                ...
                                if (window.dataLayer && window.dataLayer.length > 3) {
                                    ...
                                    for (let i = 0; i < lengthDataLayer; i++) {
                                        ...
                                        if (dataLayerObj.currentPageView && dataLayerObj.currentPageView === "HomePageView") {
                                            ...
                                            // Use Case - showcase Popup after 10 seconds and 3 minutes
                                            if (!sessionStorage.getItem('attributeValue')) {
                                                sessionStorage.setItem('attributeValue', 0);
                                            }

                                            if (parseInt(sessionStorage.getItem('attributeValue')) < 4) {
                    ...
                    contentZones: [
                        ...
                        { name: "Shop By Category", selector: ".evgShopbyCategory" },
                         ...
                         { name: "Diamond_Top_Picks", selector: ".evgDiamondCategory" },
                         { name: "Similar Search Items", selector: "#evgSimilarItemsSearch" }
                    ],
                                ...
                                if (window.dataLayer && window.dataLayer.length > 3) {
                                    ...
                                    for (let i = 0; i < lengthDataLayer; i++) {
                                        ...
                                        if (dataLayerObj.currentPageView && dataLayerObj.currentPageView === "PLPView") {
                                            console.log("On PLP");
                                             // Use Case - showcase Popup after 3 minutes on AllJewellery Page
                                            if (window.location.pathname === '/shop/jewellery') {
                                                if (!sessionStorage.getItem('allJewAttributeValue')) {
                                                    sessionStorage.setItem('allJewAttributeValue', 0);
                                                }
                                                if (parseInt(sessionStorage.getItem('allJewAttributeValue')) < 1) {
                                                   window.addEventListener("load", function() {
                                                        setTimeout(function () {
                                                            AllJewPopupCall()
                                                        }, 180000);
        ...
        function AllJewPopupCall() {       
            if (!evgloginStatus) {   
                if ($('#loginModal:visible').length === 0) {
                    let popResp = $('#loginButton').trigger('click');
                    sessionStorage.setItem('allJewAttributeValue', parseInt(sessionStorage.getItem('allJewAttributeValue')) + 1);
                }
            }
        }
        function firstPopupCall() {      
            if (!evgloginStatus) {   
                if ($('#loginModal:visible').length === 0) {
                    let firstPopupResp = $('#loginButton').trigger('click');
                    sessionStorage.setItem('attributeValue', parseInt(sessionStorage.getItem('attributeValue')) + 1);
                    countPopup = 1;
                }   
                else {
                    var currentTime1 = new Date();      
                    // Calculate the time after 3 minutes
                    var threeMinutes1 = new Date(currentTime1.getTime() + (3 * 60 * 1000));
                    setTimeout(function () {
                        if ($('#loginModal:visible').length === 0) {
                            let secondPopupResp1 = $('#loginButton').trigger('click');
                            sessionStorage.setItem('attributeValue', parseInt(sessionStorage.getItem('attributeValue')) + 1);
                        }
                    }, threeMinutes1 - currentTime1);
                }
                if (countPopup === 1) {
                    let intervalCheck = window.setInterval(function () {
                        let max = 11;
                        let current = 0;
                        if (current === max) {
                        ...
                        current++;
                        console.log($('#loginModal:visible').length);
                        if ($('#loginModal:visible').length === 1) {
                            let unVar1 = $('#loginModal:visible').find($('.loginpopup-close:visible').addClass('evgPopupLoginDismissal'));
                            var element1 = document.querySelector('.evgPopupLoginDismissal');
                            if (element1) {
        ...
        function handleClick(event) {
            var element2 = document.querySelector('.evgPopupLoginDismissal');
            if (element2) {
                element2.removeEventListener('click', handleClick);
                element2.classList.remove('evgPopupLoginDismissal');
            }
            countPopup = 0;
            if (parseInt(sessionStorage.getItem('attributeValue')) < 4) {
                var currentTime = new Date();
                // Calculate the time after 3 minutes
                var threeMinutesLater = new Date(currentTime.getTime() + (3 * 60 * 1000));
                setTimeout(function () {
                    if (!evgloginStatus) {
                        if ($('#loginModal:visible').length === 0) {
                            let secondPopupResp = $('#loginButton').trigger('click');
                            sessionStorage.setItem('attributeValue', parseInt(sessionStorage.getItem('attributeValue')) + 1);
                        }
                    }
                }, threeMinutesLater - currentTime);
            }
